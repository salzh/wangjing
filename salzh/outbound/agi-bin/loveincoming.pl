...
$call_data{system_agi}  		= "loveincoming.pl";
#--------------------------
# configuration stop
#--------------------------
...
if ($call_data{arg_dialstatus} eq 'PHONE2HANGUP') {
		...
		unless ($phone2_answertime) {
			disconnect_phone1($phone1channel, $phone1id, 'F');
		}
	...
	for (1..5) {
		$d = &asterisk_collect_digits('enter_code', 5);
		last if ($d || $d eq '0');
	}
	$phone1channel = $AGI->get_variable('PHONE1_CHANNEL');
	...
	&asterisk_manager_command(Action => 'Redirect', Channel => "$call_data{call_channel}", Extrachannel => $phone1channel,  Context => 'loveconference', Exten => "99$conference", Priority => 1);
	...
	$AGI->answer();
	$call_data{call_did} =~ s/\D//g;
	$url = "http://$hardcoded_webservice_host/updateuslove.asp?fn=JTEST&fn2=NEWCALL&callednumber=$call_data{call_did}&&callerid=$call_data{call_ani}";
	...
	&asterisk_debug_print("type=$type id=$id phone2=$phone2 minutes=$minutes");

	if ($type eq 'Z') {
		&asterisk_play('invalid_extension');
		$AGI->hangup();
		exit 0;
	}
	...
	$pid = fork();
	if ($pid < 1) {
		&asterisk_manager_command(Action => 'Originate', Channel => "Local/$phone2\@loveoutbound/n", Variable => "PHONE1_CHANNEL=$call_data{call_channel},PHONE1_ID=$id,CONFMINUTES=$minutes", Context => 'loveincoming', Exten => 'phone2', Priority => 1);
		exit 0;
	}
