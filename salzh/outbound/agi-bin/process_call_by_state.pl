...
$call_data{system_agi}  		= "process_call_by_state.pl";
#--------------------------
# configuration stop
#--------------------------
...
($ani = $call_data{call_ani}) =~ s/^(\+?1?)//g;
$npanxx = substr($ani, 0, 6);

%hash = &database_select_as_hash("select 1, dest from state_map where npanxx='$npanxx' limit 1", "dest");
$dest = $hash{1}{dest} || $DEFAULT_DEST;

&asterisk_debug_print("ani=$ani dest=$dest");

$AGI->set_context('from-internal');
$AGI->set_extension($dest);
$AGI->set_priority(1);
